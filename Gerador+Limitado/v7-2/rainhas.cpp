...
void gera(unsigned int x, float* e, unsigned int k, unsigned int* n_sol, float*** R, float** S, unsigned int*** mem_contagens, std::vector<std::set<unsigned int>>& mem_indices, unsigned int n_des)
{
    ...
    {
        // Se há possibilidade:
        if(!(mem_indices[k].empty()))
        {
            // Faz um número de tentativas igual ao número de possibilidades:
            for(unsigned int z = 1; z <= mem_indices[k].size(); z++)
            {
                // Gera um índice aleatório para acessar o conjunto de índices válidos:
                unsigned int t = std::rand() % mem_indices[k].size();
                // Inicia iterador para navegar até o índice aleatório:
                auto it = mem_indices[k].begin();
                // Avança o iterador para o índice desejado:
                std::advance(it, t);
                // Salva o índice da componente:
                unsigned int j = *it;
