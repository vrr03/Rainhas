...
void gera(unsigned int x, float* e, unsigned int bandeira, unsigned int* n_sol, float*** R, float** S, unsigned int** memoria, unsigned int n_des)
{
        ...
        for(int j = 0; j <= int(t-2); j++)
        {
            ...
            if(!(*memoria)[j])
            {
                ...
                if(i > bandeira)
                {
                    ...
                    gera(x, e, bandeira+1, n_sol, R, S, memoria, n_des);
                }
...
void gerador(unsigned int x, float* e, unsigned int* n_sol, float*** R, unsigned int n_des)
{
    ...
    for(int i = 0; i <= int(x-1); i++)
    {
        ...
        // Marca o índice como utilizado na memória:
        memoria[i] = 1;
        gera(x, e, 1, n_sol, R, &S, &memoria, n_des);
        memoria[i] = 0;
    }
