...
void gera(unsigned int x, float* e, int k, unsigned int t, unsigned int* n_sol, float*** R, float** S, unsigned int** memoria, unsigned int n_des)
{
    ...
    {
        // Faz x-k+1 tentativas:
        for(int i = 0; i <= x-k+1; i++)
        {
            // Gera um índice aleatório entre 0 e x-1 (que se refere as possibilidades do espaço de possibilidades):
            int j = std::rand()%x;
            ...
            if(j == t-k || j == t || j == t+k)
            {
                // Não é um índice válido:
                continue;
            }
            // Se a possibilidade pode ser usada como componente de uma solução parcial:
