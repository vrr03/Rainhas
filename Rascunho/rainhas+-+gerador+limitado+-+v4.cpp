...
void gera(unsigned int x, float* e, unsigned int k, unsigned int* n_sol, float*** R, unsigned int n_des, float** S, unsigned int** memoria)
{
        ...
        for(int j = 0; j <= int(t-2); j++)
        {
            ...
            if(!(*memoria)[j])
            {
                ...
                if(i == k+1)
                {
                    ...
                    gera(x, e, k+1, n_sol, R, n_des, S, memoria);
                }
...
void gerador(unsigned int x, float* e, unsigned int* n_sol, float*** R, unsigned int n_des)
{
    ...
    for(int i = 0; i <= int(x-1); i++)
    {
        ...
        // Marca o índice como utilizado na memória:
        memoria[i] = 1;
        gera(x, e, 1, n_sol, R, n_des, &S, &memoria);
        memoria[i] = 0;
    }
